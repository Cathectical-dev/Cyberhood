...
if not game:IsLoaded() then
	...
	NLoaded.Text = 'Cyberhood - Waiting'
	game.Loaded:Wait()
	NLoaded.Text = 'Cyberhood - Loaded'
	wait(1)
	NLoaded:Destroy()
...
mainDomain.TemplateBanner.Size = UDim2.new(0.123989441, 0, 0.870303035, 0)
...
SaveSettings = false
VersionNum = 0.65
VersionFull = tostring(VersionNum).." | Quality of life"
VFeature = "Small tweaks to enchance your experience"

Admins = {

	RxalKieran = 665514044,
	xMertiniOnex = 192277161,
...
SettingsTb = {
	...
	AutoExecuteScripts = {
		...
		Value = false,
		Desc = "On startup, Cyberhood will search for scripts and if they are for the game you're in, it will execute it automatically.",
...
db = false
DiscordLink = "https://discord.gg/MYHEQaaZtb"
MusicSound = {}
DomainData = {}
NotifStack = {}
...
function Start()
	...
	if table.find(Banned,game.Players.LocalPlayer.UserId) then
		mainDomain.Functions.Visible = false
	...
	if game.CreatorId ~= 0 then
		for _, UiInstance in ipairs(game:GetService("CoreGui"):GetChildren()) do
			if UiInstance.Name == "mainDomain" and UiInstance ~= mainDomain.mainDomain then
				Notify("Removed previous instance of Domain","GothamBold")
				UiInstance:Destroy()
				beforehand = true
			end
		end
	...
	wait(0.4)
	Notify("Successfully booted Cyberhood v"..tostring(VersionNum),"GothamBold")
	...
	if UsingAutoExecute == true then
		...
		if #mainDomain.MediaMain.Exploits:GetChildren() <= 4 then
	...
	wait(2)
	Notify("Cyberhood V2 is coming soon! Check the discord for more info! (and be a beta tester!)","GothamBlack")
end
...
function OpenInfo()
	...
	wait(0.25)
	mainDomain.Toggle.Visible = false
	mainDomain.Info_2.Visible = true
...
game.Players.PlayerAdded:Connect(function(Player)
	...
	if table.find(Admins,Player.UserId) then
		wait(2)
		Notify("Cyberhood Admin "..Player.Name.." has joined your server","GothamBold")
	end
end)
	...
	if game.Players:FindFirstChild(PlayerName) then
		...
		for _, adm in pairs(Admins) do
			if adm == Player.UserId then
				...
				mainDomain.AboutPlayer.DomainDev.Text = "Cyberhood Admin"
		...
		for _, shw in pairs(DomainShowcase) do
			if shw == Player.UserId then
				mainDomain.AboutPlayer.DomainDev.Visible = true
				mainDomain.AboutPlayer.DomainDev.Text = "Cyberhood Showcaser"
				mainDomain.DomainDev.Visible = true
		...
		for _, dev in pairs(Developers) do
			if dev == Player.UserId then
				mainDomain.AboutPlayer.DomainDev.Visible = true
				mainDomain.AboutPlayer.DomainDev.Text = "Cyberhood Developer"
				mainDomain.DomainDev.Visible = true
		...
		-- Animation
		wait(0.1)
		if mainDomain.AboutPlayer.DomainDev.Visible == false and mainDomain.AboutPlayer.DomainDev.Text ~= "Cyberhood Developer" and mainDomain.AboutPlayer.DomainDev.Text ~= "Cyberhood Admin" and mainDomain.AboutPlayer.DomainDev.Text ~= "Cyberhood Showcaser" then
			mainDomain.AboutPlayer.DomainDev.Visible = true
